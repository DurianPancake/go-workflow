...
// NewTask 新任务
func NewTask(t *model.Task) (string, error) {
	if len(t.NodeID) == 0 {
		return "", errors.New("request param nodeID can not be null / 任务当前所在节点nodeId不能为空！")
	}
...
// NewTaskTx NewTaskTx
// 开启事务
func NewTaskTx(t *model.Task, tx *gorm.DB) (string, error) {
	if len(t.NodeID) == 0 {
		return "", errors.New("request param nodeID can not be null / 任务当前所在节点nodeId不能为空！")
	}
	t.CreateTime = util.FormatDate(time.Now(), util.YYYY_MM_DD_HH_MM_SS)
...
func GetTaskByID(id string) (task *model.Task, err error) {
	return model.GetTaskByID(id)
}

// GetTaskLastByProInstID GetTaskLastByProInstID
func GetTaskLastByProInstID(procInstID string) (*model.Task, error) {
...
func CompleteByToken(token string, receiver *TaskReceiver) error {
	...
	err = Complete(receiver.TaskID, userinfo.ID, userinfo.Username, userinfo.Tenant, receiver.Comment, receiver.Candidate, pass)
	if err != nil {
		return err
	}
	return nil
...
// Complete Complete
// 审批
func Complete(taskID string, userID, username, tenant, comment, candidate string, pass bool) error {
	tx := model.GetTx()
	err := CompleteTaskTx(taskID, userID, username, tenant, comment, candidate, pass, tx)
	if err != nil {
		tx.Rollback()
		return err
	}
...
func UpdateTaskWhenComplete(taskID string, userID string, pass bool, tx *gorm.DB) (*model.Task, error) {
	// 获取task
...
// 执行任务
func CompleteTaskTx(taskID string, userID, username, tenant, comment, candidate string, pass bool, tx *gorm.DB) error {
	...
	if task.ActType == "and" {
		...
		yes, err := IfParticipantByTaskID(userID, tenant, taskID)
		if err != nil {
			tx.Rollback()
			return err
		}
	...
	if task.UnCompleteNum > 0 && pass == true { // 默认是全部通过
		// 添加参与人
		err := AddParticipantTx(userID, username, tenant, comment, task.ID, task.ProcInstID, task.Step, tx)
		if err != nil {
			return err
		}
		return nil
	...
	// 	return err
	// }
	err = MoveStageByProcInstID(userID, username, tenant, comment, candidate, task.ID, task.ProcInstID, task.Step, pass, tx)
	if err != nil {
		return err
	}
...
func WithDrawTaskByToken(token string, receiver *TaskReceiver) error {
	...
	return WithDrawTask(receiver.TaskID, receiver.ProcInstID, userinfo.ID, userinfo.Username, userinfo.Tenant, receiver.Comment)
}

// WithDrawTask 撤回任务
func WithDrawTask(taskID, procInstID string, userID, username, tenant, comment string) error {
	var err1, err2 error
	...
	// 撤回
	err = MoveStageByProcInstID(userID, username, tenant, comment, "", currentTask.ID, procInstID, currentTask.Step, pass, tx)
	if err != nil {
		tx.Rollback()
		return err
	}
...
func MoveStageByProcInstID(userID, username, tenant, comment, candidate, taskID, procInstID string, step int, pass bool, tx *gorm.DB) (err error) {
	nodeInfos, err := GetExecNodeInfosByProcInstID(procInstID)
	if err != nil {
		return err
	}
	return MoveStage(nodeInfos, userID, username, tenant, comment, candidate, taskID, procInstID, step, pass, tx)
}

// MoveStage MoveStage
// 流程流转
func MoveStage(nodeInfos []*flow.NodeInfo, userID, username, tenant, comment, candidate, taskID, procInstID string, step int, pass bool, tx *gorm.DB) (err error) {
	// 添加上一步的参与人
	err = AddParticipantTx(userID, username, tenant, comment, taskID, procInstID, step, tx)
	if err != nil {
		return err
	}
	if pass {
	...
	if nodeInfos[step].AproverType == flow.NodeTypes[flow.NOTIFIER] {
		...
		// 添加抄送人
		err = AddNotifierTx(nodeInfos[step].Aprover, tenant, step, procInstID, tx)
		if err != nil {
			return err
		}
		return MoveStage(nodeInfos, userID, username, tenant, comment, candidate, taskID, procInstID, step, pass, tx)
	...
	if pass {
		return MoveToNextStage(nodeInfos, userID, tenant, taskID, procInstID, step, tx)
	}
	return MoveToPrevStage(nodeInfos, userID, tenant, taskID, procInstID, step, tx)
}

// MoveToNextStage MoveToNextStage
//通过
func MoveToNextStage(nodeInfos []*flow.NodeInfo, userID, tenant, currentTaskID, procInstID string, step int, tx *gorm.DB) error {
	...
	if (step + 1) != len(nodeInfos) { // 下一步不是【结束】
		...
		// 添加candidate group
		err = AddCandidateGroupTx(nodeInfos[step].Aprover, tenant, step, taksID, procInstID, tx)
		if err != nil {
			return err
		}
		// 更新流程实例
...
// 驳回
func MoveToPrevStage(nodeInfos []*flow.NodeInfo, userID, tenant, currentTaskID, procInstID string, step int, tx *gorm.DB) error {
	// 生成新的任务
	...
	if step == 0 { // 流程回到起始位置，注意起始位置为0,
		err = AddCandidateUserTx(nodeInfos[step].Aprover, tenant, step, taksID, procInstID, tx)
		if err != nil {
			return err
		}
		return nil
...
func getNewTask(nodeInfos []*flow.NodeInfo, step int, procInstID string, currentTime string) *model.Task {

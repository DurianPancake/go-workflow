...
func SaveExecution(e *model.Execution) (ID string, err error) {
	execLock.Lock()
	...
	if yes {
		return "", errors.New("流程实例【" + fmt.Sprintf("%d", e.ProcInstID) + "】已经存在执行流")
	}
	// save
	return e.Save()
}
...
func SaveExecTx(e *model.Execution, tx *gorm.DB) (ID string, err error) {
	execLock.Lock()
	defer execLock.Unlock()
	// check if exists by procInst
	yes, err := model.ExistsExecByProcInst(e.ProcInstID)
...
// 根据流程实例查询执行流
func GetExecByProcInst(procInst string) (*model.Execution, error) {
	return model.GetExecByProcInst(procInst)
}

// GenerateExec GenerateExec
// 根据流程定义node生成执行流
func GenerateExec(e *model.Execution, node *flow.Node, userID string, variable *map[string]string, tx *gorm.DB) (string, error) {
...
// 获取执行流经过的节点信息
func GetExecNodeInfosByProcInstID(procInstID string) ([]*flow.NodeInfo, error) {
